document.addEventListener('DOMContentLoaded', () => {
    ...
    let lastMoveTime = 0;
    const cooldown = 300; // ms between moves to prevent wobble
    ...
    const moveButton = (e) => {
        ...
        // Normalize and apply jump distance
        let distance = Math.sqrt(dx * dx + dy * dy);

        // Fix for mobile/center touch: If distance is too small (touched center), pick random direction
        if (distance < 10) {
            dx = (Math.random() - 0.5) * 20; // Random small vector
            dy = (Math.random() - 0.5) * 20;
            distance = Math.sqrt(dx * dx + dy * dy);
        }

        const jumpDistance = 200;
        dx = (dx / distance) * jumpDistance;
        dy = (dy / distance) * jumpDistance;
        ...
        if (wasCornered) {
            // Jump to CENTER of screen to escape the corner!
            newX = (vw - btnWidth) / 2;
            newY = (vh - btnHeight) / 2;
        } else {
            newX = clampedX;
